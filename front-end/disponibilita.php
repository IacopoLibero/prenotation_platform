...
// Recupera le impostazioni di visualizzazione dell'utente
$useRealTime = isset($_GET['realtime']) ? ($_GET['realtime'] == 1) : true;
$displayMode = $useRealTime ? 'realtime' : 'database';
...
<body data-realtime="<?php echo $useRealTime ? 'true' : 'false'; ?>">
    <header>
        <nav>
            ...
            <div class="availability-section">
                <?php if($has_google_calendar): ?>
                    <div class="status-box sync">
                        <p><strong><?php echo $useRealTime ? "Google Calendar sincronizzato in tempo reale ✓" : "Google Calendar sincronizzato ✓"; ?></strong></p>
                        <p>
                            <?php if($useRealTime): ?>
                                Stai visualizzando la disponibilità in tempo reale dal tuo Google Calendar.
                            <?php else: ?>
                                Stai sincronizzando la tua disponibilità con Google Calendar. Le nuove lezioni saranno generate in base al tuo calendario.
                            <?php endif; ?>
                        </p>
                        <div class="display-mode-selector">
                            <label for="viewMode">Modalità visualizzazione:</label>
                            <select id="viewMode" onchange="toggleViewMode(this.value)">
                                <option value="realtime" <?php echo $useRealTime ? 'selected' : ''; ?>>Tempo reale</option>
                                <option value="database" <?php echo !$useRealTime ? 'selected' : ''; ?>>Database</option>
                            </select>
                        </div>
                        <button id="syncBtn" class="btn-sync">
                            ...
                            <?php echo $useRealTime ? 'Aggiorna dati in tempo reale' : 'Sincronizza database'; ?>
                        </button>
                ...
                <h2>Disponibilità <?php echo $useRealTime ? 'in tempo reale' : 'dal database'; ?></h2>
                
                <?php if($has_google_calendar && $useRealTime): ?>
                ...
                <?php else: ?>
                    <?php
                    // Se non stiamo usando la modalità in tempo reale, carica dal database come prima
                    ...
                    $has_any_availability = !empty($availability_by_week);
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            <?php if(!$useRealTime && isset($availability_by_week) && !empty($availability_by_week)): ?>
        ...
        function toggleViewMode(mode) {
            window.location.href = '?realtime=' + (mode === 'realtime' ? '1' : '0');
        }
    </script>
</body>
</html>